...
const createAlumno = async (req, res) => {
    const { nombre, apellido, numero, correo, carrera, status, votacionId } = req.body
    ...
    // }
        ...
        newAlumno.save((error, alumno) => {
            if (error) {
                ...
                return res.status(400).send({ message: "No se ha podido crear el alumno" })
            }
...
const changeStatus = (req, res) => {
    ...
    query.exec((error, alumno) => {
        if(error){
            return res.status(400).send({ message: "No se pudo actualizar el alumno 1" })
        }
        if(alumno.status === 'Bloqueado'){
            alumno.updateOne({status: 'Permitido'}).exec((error) => {
                if(error){
                    return res.status(400).send({ message: "No se pudo actualizar el alumno 2" })
                }
            ...
            alumno.updateOne({status: 'Bloqueado'}).exec((error) => {
                if(error){
                    return res.status(400).send({ message: "No se pudo actualizar el alumno :"+error })
                }
...
const updateAlumno = (req, res) => {
    ...
    Alumno.findByIdAndUpdate(id, req.body, (error, alumno) => {
        if (error) {
            return res.status(400).send({ message: "No se pudo actualizar el alumno" })
        }
        if (!alumno) {
...
const deleteAlumno = (req, res) => {
    ...
    Alumno.findByIdAndDelete(id, (error, alumno) => {
        if (error) {
            return res.status(400).send({ message: "No se ha podido eliminar el alumno" })
        }
        if (!alumno) {
            return res.status(404).send({ message: "No se ha podido encontrar un alumno" })
        }
        return res.status(200).send({ message: "Se ha eliminado el alumno de forma correcta" })
    })
}
...
const getAlumno = (req, res) => {
    ...
    Alumno.findById(id, (error, alumno) => {
        if (error) {
            return res.status(400).send({ message: "No se ha podido modificar el alumno" })
        }
        if (!alumno) {
            return res.status(404).send({ message: "No se ha podido encontrar un alumno" })
        }
